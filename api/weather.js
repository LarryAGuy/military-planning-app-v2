 ...
 *
 * Purpose: Securely proxy weather API requests to OpenWeatherMap API
...
export default async function handler(req, res) {
    ...
    if (!lat || !lon) {
        return res.status(400).json({
            ...
            example: '/api/weather?lat=38.8977&lon=-77.0365&units=imperial'
        });
    }
    ...
    try {
        // Call OpenWeatherMap API with server-side API key
        const apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${validation.lat}&lon=${validation.lon}&appid=${WEATHER_API_KEY}&units=${units}`;
        ...
        if (!response.ok) {
            ...
            return res.status(response.status).json({
                ...
                message: errorData.message || 'Failed to fetch weather data from external API',

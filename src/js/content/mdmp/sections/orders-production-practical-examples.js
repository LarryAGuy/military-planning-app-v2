/** MDMP Step 7: Orders Production - Practical Examples (Green theme) */
export const ordersProductionPracticalExamplesModule = {
  id: 'orders-production-practical-examples',
  title: 'Practical Examples',
  defaultExpanded: false,
  getContent() {
    return `
      <div id="orders-production-practical-examples-section" style="margin-bottom:24px;">
        ...
        <div id="orders-production-practical-content" style="min-height:300px;">
  ...
  initializeDropdowns() {
    document.querySelectorAll('#orders-production-practical-examples-section .echelon-dropdown').forEach(dropdown => {
  ...
  initializeButtons() {
    document.querySelectorAll('#orders-production-practical-examples-section .example-tab-btn').forEach(btn => {
  ...
  injectDropdownCSS() {
    const style = document.getElementById('orders-production-practical-dropdown-styles') || document.createElement('style');
    style.id = 'orders-production-practical-dropdown-styles';
    style.textContent = `
      #orders-production-practical-examples-section .echelon-dropdown-btn:hover { color:#10b981 !important; transition: color .3s ease !important; }
      #orders-production-practical-examples-section .dropdown-item:hover { background-color:#374151 !important; transition: all .2s ease !important; }
      #orders-production-practical-examples-section .dropdown-content { transition: opacity .2s ease !important; z-index:1000 !important; position:absolute !important; top:100% !important; left:0 !important; background-color:#1f2937 !important; border:1px solid #374151 !important; border-radius:6px !important; box-shadow:0 4px 12px rgba(0,0,0,.3) !important; min-width:200px !important; padding:8px !important; }
      #orders-production-practical-examples-section .echelon-dropdown { position: relative !important; display: inline-block !important; }
      #orders-production-practical-examples-section .echelon-dropdown:hover .dropdown-content { display: block !important; }
    `;
    const existing = document.getElementById('orders-production-practical-dropdown-styles');
  ...
  async loadEchelonExample(echelon) {
    const contentArea = document.getElementById('orders-production-practical-content');
    ...
    catch (error) { console.error('Failed to load Orders Production practical example:', error); contentArea.innerHTML = '<div style="text-align:center; padding:40px; color:#ef4444;"><i class="fas fa-exclamation-triangle" style="font-size:24px; margin-bottom:12px;"></i><p>Failed to load content</p></div>'; }
  },
  async importEchelon(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./orders-production-practical-platoon.js'); return m.getPlatoonOrdersProductionExample(); }
      case 'company': { const m = await import('./orders-production-practical-company.js'); return m.getCompanyOrdersProductionExample(); }
      case 'squadron': { const m = await import('./orders-production-practical-squadron.js'); return m.getSquadronOrdersProductionExample(); }
      case 'battalion': { const m = await import('./orders-production-practical-battalion.js'); return m.getBattalionOrdersProductionExample(); }
      case 'regiment': { const m = await import('./orders-production-practical-regiment.js'); return m.getRegimentOrdersProductionExample(); }
      case 'group': { const m = await import('./orders-production-practical-group.js'); return m.getGroupOrdersProductionExample(); }
      case 'brigade': { const m = await import('./orders-production-practical-brigade.js'); return m.getBrigadeOrdersProductionExample(); }
      case 'division': { const m = await import('./orders-production-practical-division.js'); return m.getDivisionOrdersProductionExample(); }
      case 'corps': { const m = await import('./orders-production-practical-corps.js'); return m.getCorpsOrdersProductionExample(); }
      case 'army': { const m = await import('./orders-production-practical-army.js'); return m.getArmyOrdersProductionExample(); }
      case 'theater': { const m = await import('./orders-production-practical-theater.js'); return m.getTheaterOrdersProductionExample(); }
  ...
  initialize() {
    ...
    this.documentClickHandler = (e) => {
      if (document.getElementById('orders-production-practical-content') && !e.target.closest('.echelon-dropdown') && !e.target.closest('#sidebar') && !e.target.closest('#right-sidebar')) { this.hideAll(); }
    };
...
export default ordersProductionPracticalExamplesModule;

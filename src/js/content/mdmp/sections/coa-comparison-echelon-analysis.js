/**
 * MDMP Step 5: COA Comparison - Echelon Analysis (Purple theme)
 * Framework scaffolding only - no content population
 */

export const coaComparisonEchelonAnalysisModule = {
  id: 'coa-comparison-echelon-analysis',
  title: 'Echelon Analysis',
  defaultExpanded: false,

  getContent() {
    return `
      <div id="coa-comparison-echelon-analysis-section" style="margin-bottom:24px;">
        ...
        <div id="coa-comparison-echelon-content" style="min-height:300px;">
  ...
  initializeDropdowns() {
    document.querySelectorAll('#coa-comparison-echelon-analysis-section .echelon-dropdown').forEach(dropdown => {
  ...
  initializeButtons() {
    document.querySelectorAll('#coa-comparison-echelon-analysis-section .example-tab-btn').forEach(btn => {
  ...
  injectDropdownCSS() {
    const style = document.getElementById('coa-comparison-echelon-dropdown-styles') || document.createElement('style');
    style.id = 'coa-comparison-echelon-dropdown-styles';
    style.textContent = `
      /* Scoped to COA Comparison Echelon Analysis section to avoid sidebar collisions */
      #coa-comparison-echelon-analysis-section .echelon-dropdown-btn:hover { color:#10b981 !important; transition: color .3s ease !important; }
      #coa-comparison-echelon-analysis-section .dropdown-item:hover { background-color:#374151 !important; transition: all .2s ease !important; }
      #coa-comparison-echelon-analysis-section .dropdown-content { 
      ...
      #coa-comparison-echelon-analysis-section .echelon-dropdown { position: relative !important; display: inline-block !important; }
      #coa-comparison-echelon-analysis-section .echelon-dropdown:hover .dropdown-content { display: block !important; }
    `;
    const existing = document.getElementById('coa-comparison-echelon-dropdown-styles');
  ...
  async loadEchelonAnalysis(echelon) {
    const contentArea = document.getElementById('coa-comparison-echelon-content');
    ...
    catch (error) { console.error('Failed to load COA comparison echelon analysis:', error); contentArea.innerHTML = '<div style="text-align:center; padding:40px; color:#ef4444;"><i class="fas fa-exclamation-triangle" style="font-size:24px; margin-bottom:12px;"></i><p>Failed to load content</p></div>'; }
  },

  async importEchelonAnalysis(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./coa-comparison-analysis-platoon.js'); return m.platoonCOAComparisonModule.getContent(); }
      case 'company': { const m = await import('./coa-comparison-analysis-company.js'); return m.companyCOAComparisonModule.getContent(); }
      case 'squadron': { const m = await import('./coa-comparison-analysis-squadron.js'); return m.squadronCOAComparisonModule.getContent(); }
      case 'battalion': { const m = await import('./coa-comparison-analysis-battalion.js'); return m.battalionCOAComparisonModule.getContent(); }
      case 'regiment': { const m = await import('./coa-comparison-analysis-regiment.js'); return m.regimentCOAComparisonModule.getContent(); }
      case 'group': { const m = await import('./coa-comparison-analysis-group.js'); return m.groupCOAComparisonModule.getContent(); }
      case 'brigade': { const m = await import('./coa-comparison-analysis-brigade.js'); return m.brigadeCOAComparisonModule.getContent(); }
      case 'division': { const m = await import('./coa-comparison-analysis-division.js'); return m.divisionCOAComparisonModule.getContent(); }
      case 'corps': { const m = await import('./coa-comparison-analysis-corps.js'); return m.corpsCOAComparisonModule.getContent(); }
      case 'army': { const m = await import('./coa-comparison-analysis-army.js'); return m.armyCOAComparisonModule.getContent(); }
      case 'theater': { const m = await import('./coa-comparison-analysis-theater.js'); return m.theaterCOAComparisonModule.getContent(); }
  ...
  initialize() {
    ...
    this.documentClickHandler = (e) => {
      if (document.getElementById('coa-comparison-echelon-content') &&
...
export default coaComparisonEchelonAnalysisModule;

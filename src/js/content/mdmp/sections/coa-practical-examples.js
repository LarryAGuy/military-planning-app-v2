/**
 * MDMP Course of Action Development - Practical Examples
 * Three-tier dropdown navigation system for COA development practical examples
 * Mirrors MDMP Mission Analysis practical examples architecture
 * Teal/cyan theme (#0891b2) for COA Development
 */

export const coaPracticalExamplesModule = {
    id: 'coa-practical-examples',
    title: 'Practical Examples',
    defaultExpanded: false,

    getContent() {
        return `
            <div id="coa-practical-examples-section" class="dashboard-widgets mb-12">
                ...
                <div class="example-tab-content" style="background-color:#1f2937; border-radius:8px; border:1px solid #374151; padding:24px; margin-top:16px;">
                    <div style="text-align:center; padding:40px; color:#6b7280;">
                        <i class="fas fa-route" style="font-size:48px; margin-bottom:16px; color:#0891b2;"></i>
                        <h3 style="font-size:18px; font-weight:600; margin-bottom:8px; color:#d1d5db;">COA Development Practical Examples</h3>
                        <p style="font-size:14px; line-height:1.5; margin-bottom:16px;">Select an echelon level from the dropdowns above to view course of action development examples.</p>
                        
                        <!-- Framework Status -->
                        <div style="background-color:#374151; padding:16px; border-radius:8px; border-left:4px solid #0891b2; margin-top:20px; text-align:left;">
                            <h4 style="color:#67e8f9; font-size:14px; font-weight:600; margin:0 0 8px 0;">
                            ...
                            <p style="color:#d1d5db; font-size:12px; line-height:1.5; margin:0 0 12px 0;">
                                All Strategic, Operational, and Tactical COA development examples implemented with complete hierarchical scaffolding
    ...
    injectDropdownCSS() {
        const style = document.getElementById('coa-practical-dropdown-hover-styles') || document.createElement('style');
        style.id = 'coa-practical-dropdown-hover-styles';
        style.textContent = `
            /* COA practical examples dropdown hover effects */
            .echelon-dropdown-btn:hover { color:#16a34a !important; }
        ...
        `;
        const existing = document.getElementById('coa-practical-dropdown-hover-styles');
    ...
    async loadEchelonExample(echelon) {
        ...
        // Load modular COA development examples for Strategic, Operational, and Tactical echelons
        switch (echelon) {
            case 'division':
                const { getDivisionCOAExample } = await import('./coa-practical-division.js');
                content.innerHTML = getDivisionCOAExample();
                return;
            case 'corps':
                const { getCorpsCOAExample } = await import('./coa-practical-corps.js');
                content.innerHTML = getCorpsCOAExample();
                return;
            case 'army':
                const { getArmyCOAExample } = await import('./coa-practical-army.js');
                content.innerHTML = getArmyCOAExample();
                return;
            case 'theater':
                const { getTheaterCOAExample } = await import('./coa-practical-theater.js');
                content.innerHTML = getTheaterCOAExample();
                return;
            case 'regiment':
                const { getRegimentCOAExample } = await import('./coa-practical-regiment.js');
                content.innerHTML = getRegimentCOAExample();
                return;
            case 'group':
                const { getGroupCOAExample } = await import('./coa-practical-group.js');
                content.innerHTML = getGroupCOAExample();
                return;
            case 'brigade':
                const { getBrigadeCOAExample } = await import('./coa-practical-brigade.js');
                content.innerHTML = getBrigadeCOAExample();
                return;
            case 'platoon':
                const { getPlatoonCOAExample } = await import('./coa-practical-platoon.js');
                content.innerHTML = getPlatoonCOAExample();
                return;
            case 'company':
                const { getCompanyCOAExample } = await import('./coa-practical-company.js');
                content.innerHTML = getCompanyCOAExample();
                return;
            case 'squadron':
                const { getSquadronCOAExample } = await import('./coa-practical-squadron.js');
                content.innerHTML = getSquadronCOAExample();
                return;
            case 'battalion':
                const { getBattalionCOAExample } = await import('./coa-practical-battalion.js');
                content.innerHTML = getBattalionCOAExample();
                return;
            default:
...
export default coaPracticalExamplesModule;

/**
 * MDMP Mission Analysis - Echelon Analysis
 * Three-tier dropdown navigation system for mission analysis echelon analysis
 * Mirrors MDMP Receipt of Mission echelon analysis architecture
 */
...
export const echelonAnalysisModule = {
            ...
            <div id="mission-echelon-analysis-section" class="dashboard-widgets mb-12">
                ...
                <div class="analysis-tab-content" style="background-color:#1f2937; border-radius:8px; border:1px solid #374151; padding:24px; margin-top:16px;">
                    <div style="text-align:center; padding:40px; color:#6b7280;">
                        ...
                        <h3 style="font-size:18px; font-weight:600; margin-bottom:8px; color:#d1d5db;">Mission Analysis Echelon Analysis</h3>
    ...
    injectDropdownCSS() {
        const style = document.getElementById('mission-analysis-dropdown-hover-styles') || document.createElement('style');
        style.id = 'mission-analysis-dropdown-hover-styles';
        style.textContent = `
            /* Mission echelon analysis dropdown hover effects */
        ...
        `;
        const existing = document.getElementById('mission-analysis-dropdown-hover-styles');
    ...
    async loadEchelonAnalysis(echelon) {
        ...
        try {
            // Load modular mission analysis for Strategic, Operational, and Tactical echelons
            switch (echelon) {
                case 'division':
                    const { divisionMissionAnalysisModule } = await import('./mission-analysis-division.js');
                    content.innerHTML = divisionMissionAnalysisModule.getContent();
                    return;
                case 'corps':
                    const { corpsMissionAnalysisModule } = await import('./mission-analysis-corps.js');
                    content.innerHTML = corpsMissionAnalysisModule.getContent();
                    return;
                case 'army':
                    const { armyMissionAnalysisModule } = await import('./mission-analysis-army.js');
                    content.innerHTML = armyMissionAnalysisModule.getContent();
                    return;
                case 'theater':
                    const { theaterMissionAnalysisModule } = await import('./mission-analysis-theater.js');
                    content.innerHTML = theaterMissionAnalysisModule.getContent();
                    return;
                case 'regiment':
                    const { regimentMissionAnalysisModule } = await import('./mission-analysis-regiment.js');
                    content.innerHTML = regimentMissionAnalysisModule.getContent();
                    return;
                case 'group':
                    const { groupMissionAnalysisModule } = await import('./mission-analysis-group.js');
                    content.innerHTML = groupMissionAnalysisModule.getContent();
                    return;
                case 'brigade':
                    const { brigadeMissionAnalysisModule } = await import('./mission-analysis-brigade.js');
                    content.innerHTML = brigadeMissionAnalysisModule.getContent();
                    return;
                case 'platoon':
                    const { platoonMissionAnalysisModule } = await import('./mission-analysis-platoon.js');
                    content.innerHTML = platoonMissionAnalysisModule.getContent();
                    return;
                case 'company':
                    const { companyMissionAnalysisModule } = await import('./mission-analysis-company.js');
                    content.innerHTML = companyMissionAnalysisModule.getContent();
                    return;
                case 'squadron':
                    const { squadronMissionAnalysisModule } = await import('./mission-analysis-squadron.js');
                    content.innerHTML = squadronMissionAnalysisModule.getContent();
                    return;
                case 'battalion':
                    const { battalionMissionAnalysisModule } = await import('./mission-analysis-battalion.js');
                    content.innerHTML = battalionMissionAnalysisModule.getContent();
                    return;
                default:
            ...
            console.error(`Failed to load ${echelon} mission analysis:`, error);
            content.innerHTML = `
                <div style="padding:24px;">
                    <div style="display:flex; align-items:center; margin-bottom:16px;">
                        <i class="fas fa-exclamation-triangle" style="margin-right:12px; color:#f59e0b; font-size:20px;"></i>
                    ...
                    <div style="background-color:#374151; padding:16px; border-radius:8px; border-left:4px solid #f59e0b;">
                        <p style="color:#d1d5db; font-size:12px; line-height:1.5; margin:0 0 12px 0;">
                            Failed to load ${echelon} level mission analysis module. Please check console for details.
                        </p>

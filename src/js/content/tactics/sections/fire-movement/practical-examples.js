/** Tactics: Fire & Movement - Practical Examples (Green theme) */
export const fireMovementPracticalExamplesTacticsModule = {
  id: 'fire-movement-practical-examples',
  ...
  getContent(){
    return `
      <div id="fm-practical-examples-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="fm-practical-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-clipboard-list" style="font-size:48px; color:#10b981; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Fire & Movement - Practical Examples</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('fm-practical-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'fm-practical-dropdown-styles';
    style.textContent = `
  ...
  async loadEchelon(tab) {
    ...
    this._activeTab = tab;
    localStorage.setItem('fm-practical-last-tab', tab);
    const contentArea = document.getElementById('fm-practical-content');
    ...
    try {
      ...
      console.error('Failed to load FM practical example:', err);
...
export default fireMovementPracticalExamplesTacticsModule;

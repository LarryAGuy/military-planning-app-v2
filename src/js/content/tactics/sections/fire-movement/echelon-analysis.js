/** Tactics: Fire & Movement - Echelon Analysis (Purple theme) */
export const fireMovementEchelonAnalysisTacticsModule = {
  id: 'fire-movement-echelon-analysis',
  ...
  getContent(){
    return `
      <div id="fm-echelon-analysis-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="fm-echelon-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-layer-group" style="font-size:48px; color:#a855f7; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Fire & Movement - Echelon Analysis</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('fm-echelon-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'fm-echelon-dropdown-styles';
    style.textContent = `
  ...
  loadEchelon(tab) {
    ...
    this._activeTab = tab;
    const area = document.getElementById('fm-echelon-content');
    if (!area) return;
    ...
    try {
      ...
      console.error('Failed to load FM echelon analysis:', err);
...
export default fireMovementEchelonAnalysisTacticsModule;

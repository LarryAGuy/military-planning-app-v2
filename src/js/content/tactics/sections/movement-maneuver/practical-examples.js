/** Tactics: Movement & Maneuver - Practical Examples (Green theme) */
export const movementManeuverPracticalExamplesTacticsModule = {
  id: 'movement-maneuver-practical-examples',
  ...
  getContent(){
    return `
      <div id="mm-practical-examples-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="mm-practical-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-clipboard-list" style="font-size:48px; color:#10b981; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Movement & Maneuver - Practical Examples</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('mm-practical-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'mm-practical-dropdown-styles';
    style.textContent = `
  ...
  async loadEchelon(tab) {
    ...
    this._activeTab = tab;
    localStorage.setItem('mm-practical-last-tab', tab);
    const contentArea = document.getElementById('mm-practical-content');
    ...
    try {
      ...
      console.error('Failed to load MM practical example:', err);
      contentArea.innerHTML = '<div class="text-red-400">Failed to load content</div>';
    }
  },
  async switch(tab){
    if (this._activeTab === tab) return;
    this._activeTab = tab; localStorage.setItem('mm-practical-last-tab', tab);
    const contentArea = document.getElementById('mm-practical-content'); if (!contentArea) return;
    contentArea.innerHTML = '<div style="text-align:center; padding:40px;"><i class="fas fa-spinner fa-spin" style="font-size:18px; color:#16a34a;"></i><p class="text-gray-400">Loading...</p></div>';
    try {
      const html = await this.importExample(tab);
      contentArea.innerHTML = html;
...
export default movementManeuverPracticalExamplesTacticsModule;

/** Tactics: Urban Operations - Practical Examples (Green theme) */
export const urbanOperationsPracticalExamplesTacticsModule = {
  id: 'urban-operations-practical-examples',
  ...
  getContent(){
    return `
      <div id="uo-practical-examples-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="uo-practical-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-clipboard-list" style="font-size:48px; color:#10b981; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Urban Operations - Practical Examples</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('uo-practical-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'uo-practical-dropdown-styles';
    style.textContent = `
  ...
  async loadEchelon(tab) {
    ...
    this._activeTab = tab;
    localStorage.setItem('uo-practical-last-tab', tab);
    const contentArea = document.getElementById('uo-practical-content');
    ...
    try {
      ...
      console.error('Failed to load UO practical example:', err);
...
export default urbanOperationsPracticalExamplesTacticsModule;

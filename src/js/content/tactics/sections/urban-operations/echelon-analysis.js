/** Tactics: Urban Operations - Echelon Analysis (Purple theme) */
export const urbanOperationsEchelonAnalysisTacticsModule = {
  id: 'urban-operations-echelon-analysis',
  ...
  getContent(){
    return `
      <div id="uo-echelon-analysis-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="uo-echelon-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-layer-group" style="font-size:48px; color:#a855f7; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Urban Operations - Echelon Analysis</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('uo-echelon-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'uo-echelon-dropdown-styles';
    style.textContent = `
  ...
  loadEchelon(tab) {
    ...
    this._activeTab = tab;
    const area = document.getElementById('uo-echelon-content');
    if (!area) return;
    ...
    try {
      ...
      console.error('Failed to load UO echelon analysis:', err);
...
export default urbanOperationsEchelonAnalysisTacticsModule;

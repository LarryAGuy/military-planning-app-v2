/** Tactics: Offensive Operations - Practical Examples (Green theme) */
export const offensiveOperationsPracticalExamplesTacticsModule = {
  id: 'offensive-operations-practical-examples',
  ...
  getContent(){
    return `
      <div id="oo-practical-examples-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="oo-practical-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-clipboard-list" style="font-size:48px; color:#10b981; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Offensive Operations - Practical Examples</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('oo-practical-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'oo-practical-dropdown-styles';
    style.textContent = `
  ...
  async loadEchelon(tab) {
    ...
    this._activeTab = tab;
    localStorage.setItem('oo-practical-last-tab', tab);
    const contentArea = document.getElementById('oo-practical-content');
    ...
    try {
      ...
      console.error('Failed to load OO practical example:', err);
...
export default offensiveOperationsPracticalExamplesTacticsModule;

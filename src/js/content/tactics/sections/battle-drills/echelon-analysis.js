/** Tactics: Battle Drills - Echelon Analysis (Purple theme) */
export const battleDrillsEchelonAnalysisTacticsModule = {
  id: 'battle-drills-echelon-analysis',
  ...
  getContent(){
    return `
      <div id="bd-echelon-analysis-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="bd-echelon-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-layer-group" style="font-size:48px; color:#a855f7; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Battle Drills - Echelon Analysis</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('bd-echelon-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'bd-echelon-dropdown-styles';
    style.textContent = `
  ...
  loadEchelon(tab) {
    ...
    this._activeTab = tab;
    const area = document.getElementById('bd-echelon-content');
    if (!area) return;
    ...
    try {
      ...
      console.error('Failed to load BD echelon analysis:', err);
      area.innerHTML = '<div class="text-red-400">Failed to load content</div>';
    }
  },
  async switch(tab){
    if (this._activeTab === tab) return; this._activeTab = tab;
    const area = document.getElementById('bd-echelon-content'); if (!area) return;
    area.innerHTML = '<div style="text-align:center; padding:40px;"><i class="fas fa-spinner fa-spin" style="font-size:18px; color:#7c3aed;"></i><p class="text-gray-400">Loading...</p></div>';
    try { const html = await this.importEchelon(tab); area.innerHTML = html; }
    catch(err){ console.error('Failed to load BD echelon analysis:', err); area.innerHTML = '<div class="text-red-400">Failed to load content</div>'; }
  },
...
export default battleDrillsEchelonAnalysisTacticsModule;

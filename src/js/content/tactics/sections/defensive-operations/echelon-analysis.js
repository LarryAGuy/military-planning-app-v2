/** Tactics: Defensive Operations - Echelon Analysis (Purple theme) */
export const defensiveOperationsEchelonAnalysisTacticsModule = {
  id: 'defensive-operations-echelon-analysis',
  ...
  getContent(){
    return `
      <div id="do-echelon-analysis-section" style="margin-bottom:24px;">
        ...
        <!-- Content Area -->
        <div id="do-echelon-content" style="min-height:300px; background:#1f2937; border:1px solid #374151; border-radius:8px; padding:40px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
          <i class="fas fa-layer-group" style="font-size:48px; color:#a855f7; margin-bottom:16px; opacity:0.5;"></i>
          <h4 style="color:#d1d5db; font-size:18px; font-weight:600; margin:0 0 8px 0;">Defensive Operations - Echelon Analysis</h4>
  ...
  injectDropdownCSS() {
    if (document.getElementById('do-echelon-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'do-echelon-dropdown-styles';
    style.textContent = `
  ...
  loadEchelon(tab) {
    ...
    this._activeTab = tab;
    const area = document.getElementById('do-echelon-content');
    if (!area) return;
    ...
    try {
      ...
      console.error('Failed to load DO echelon analysis:', err);
...
export default defensiveOperationsEchelonAnalysisTacticsModule;

/**
 * Universal OPORD Mission Button System
 * Lightweight version mirroring Situation behavior (<=300 lines)
 * - Injects mission-specific CSS classes
 * - Renders three-section nav (Detailed, Practical, Echelon)
 * - Loads mission section modules dynamically
 * - Targets #universal-mission-nav and #mission-content-area
 */

export class UniversalMissionButtons {
  constructor() {
    this.initialized = false;
    this.currentSection = 'detailed-analysis';
    this.modules = {};
    this.buttonConfig = [
      { id: 'universal-mission-detailed-btn', moduleType: 'detailed-analysis', label: 'Detailed Analysis', icon: 'fas fa-search', colorClass: 'universal-mission-btn-blue', isDefault: true },
      { id: 'universal-mission-practical-btn', moduleType: 'practical-examples', label: 'Practical Examples', icon: 'fas fa-file-alt', colorClass: 'universal-mission-btn-green', isDefault: false },
      { id: 'universal-mission-echelon-btn', moduleType: 'echelon-analysis', label: 'Echelon Analysis', icon: 'fas fa-layer-group', colorClass: 'universal-mission-btn-purple', isDefault: false }
    ];
  }

  async initialize() {
    if (this.initialized) return;
    if (window.__missionButtonsInitializing) return; // prevent parallel inits (Firefox)
    window.__missionButtonsInitializing = true;
    try {
      await this.loadMissionModules();
      this.injectCSS();
      ...
      window.__missionButtonsInitializing = false;
    }
  }
  ...
  prefetchSecondaryModules() {
    // Yield to paint, then prefetch practical/echelon to match Situation speed
    const prefetch = () => {
      try {
  ...
  injectCSS() {
    if (document.getElementById('universal-mission-styles')) return;
    const style = document.createElement('style');
    style.id = 'universal-mission-styles';
    style.textContent = `
  ...
  async findContainer() {
    // Try multiple strategies similar to Situation for robustness
    for (let i = 0; i < 15; i++) {
      // Method 1: direct
      let c = document.getElementById('universal-mission-nav');
      if (c) return c;

      // Method 2: querySelector
      c = document.querySelector('#universal-mission-nav');
      if (c) return c;

      // Method 3: search inside workspace
      const workspace = document.getElementById('workspace-content');
      if (workspace) {
        c = workspace.querySelector('#universal-mission-nav');
        if (c) return c;

        // Method 4: deep search with exact id
        const all = workspace.querySelectorAll('div[id="universal-mission-nav"]');
    ...
    // Final fallback: try to create within the mission module wrapper if present
  ...
  createFallbackContainer() {
    ...
    // Prefer to insert right before mission-content-area within the mission module
    const missionArea = workspace.querySelector('#mission-content-area');
    if (missionArea && missionArea.parentElement) {
      const fallback = document.createElement('div');
      fallback.id = 'universal-mission-nav';
      fallback.style.marginBottom = '16px';
      missionArea.parentElement.insertBefore(fallback, missionArea);
      return fallback;
    }
    return null;
  }
  ...
  async createButtonHTML() {
    ...
    if (!nav) throw new Error('Mission nav container not found');
  ...
  loadDefault() {
    ...
    const content = document.getElementById('mission-content-area');
    if (content) {
  ...
  updateButtonStates(active) {
    this.buttonConfig.forEach(cfg => {
      const el = document.getElementById(cfg.id);
      if (!el) return;
      el.classList.remove('universal-mission-btn-blue', 'universal-mission-btn-green', 'universal-mission-btn-purple', 'universal-mission-btn-inactive');
      el.classList.add(cfg.moduleType === active ? cfg.colorClass : 'universal-mission-btn-inactive');
    });
  }
...
window.universalMissionButtons = null;

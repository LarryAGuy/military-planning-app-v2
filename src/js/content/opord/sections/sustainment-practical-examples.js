/**
 * OPORD Sustainment - Practical Examples (Framework Placeholder)
 * - Collapsed by default
 * - Header row with echelon dropdowns (hover behavior)
 * - LocalStorage key: opord-sustainment-practical-examples
 */

export const practicalExamplesModule = {
  id: 'sustainment-practical-examples',
  title: 'Practical Examples',
  defaultExpanded: false,

  getContent() {
    return `
      <div id="sustainment-practical-examples-section" class="dashboard-widgets mb-12">
        ...
        <div style="display:flex; align-items:center; margin-bottom:16px; gap:16px; position:relative;">
          ...
          ${['tactical','operational','strategic'].map(group => `
            <div class="echelon-dropdown" style="position:relative; display:inline-block;">
              <button id="sustainment-${group}-dropdown-btn" class="echelon-dropdown-btn" data-group="${group}" style="background:none; border:none; color:${group==='tactical'?'#10b981':'#9ca3af'}; font-size:14px; font-weight:600; cursor:pointer; padding:7px 14px; display:flex; align-items:center; transition:color .3s ease;">
                ${group.charAt(0).toUpperCase()+group.slice(1)} Level
                <i class="fas fa-chevron-down" style="margin-left:6px; font-size:12px;"></i>
              </button>
              <div id="sustainment-${group}-dropdown-content" class="dropdown-content" style="display:none; position:absolute; top:100%; left:0; background-color:#1f2937; border:1px solid #374151; border-radius:6px; box-shadow:0 4px 12px rgba(0,0,0,.3); z-index:1000; min-width:200px; padding:8px;">
  ...
  initialize() {
    ...
    try { localStorage.setItem('opord-sustainment-practical-examples', 'false'); } catch {}
    return true;
  },

  show(group) { const el = document.getElementById(`sustainment-${group}-dropdown-content`); if (el) el.style.display = 'block'; },
  hide(group) { const el = document.getElementById(`sustainment-${group}-dropdown-content`); if (el) el.style.display = 'none'; },
  ...
  updateDropdownState(active) {
    ...
    ['tactical','operational','strategic'].forEach(g => {
      const btn = document.getElementById(`sustainment-${g}-dropdown-btn`);
  ...
  async loadEchelon(tab) {
    ...
    try {
      ...
      const moduleContent = await this.importEchelon(tab);
      if (moduleContent) {
        area.innerHTML = moduleContent;
        console.log(`✅ Sustainment ${tab} content loaded successfully`);
      } else {
        throw new Error(`No content available for ${tab}`);
      }
    } catch (e) {
      console.error(`❌ Failed to load sustainment ${tab} content:`, e);
      area.innerHTML = `<div style="padding:20px; background-color:#1f2937; border-radius:8px; border:1px solid #374151;">
        ...
        <p style="color:#d1d5db; font-size:14px; line-height:1.5; margin-bottom:16px;">UNCLASSIFIED - Unable to load ${tab.toUpperCase()} level content. Please check console for details.</p>
      </div>`;
    }
  },
  ...
  async importEchelon(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./sustainment-practical-platoon.js'); return m.platoonPracticalModule.getContent(); }
      case 'company': { const m = await import('./sustainment-practical-company.js'); return m.companyPracticalModule.getContent(); }
      case 'squadron': { const m = await import('./sustainment-practical-squadron.js'); return m.squadronPracticalModule.getContent(); }
      case 'battalion': { const m = await import('./sustainment-practical-battalion.js'); return m.battalionPracticalModule.getContent(); }
      case 'regiment': { const m = await import('./sustainment-practical-regiment.js'); return m.regimentPracticalModule.getContent(); }
      case 'group': { const m = await import('./sustainment-practical-group.js'); return m.groupPracticalModule.getContent(); }
      case 'brigade': { const m = await import('./sustainment-practical-brigade.js'); return m.brigadePracticalModule.getContent(); }
      case 'division': { const m = await import('./sustainment-practical-division.js'); return m.divisionPracticalModule.getContent(); }
      case 'corps': { const m = await import('./sustainment-practical-corps.js'); return m.corpsPracticalModule.getContent(); }
      case 'army': { const m = await import('./sustainment-practical-army.js'); return m.armyPracticalModule.getContent(); }
      case 'theater': { const m = await import('./sustainment-practical-theater.js'); return m.theaterPracticalModule.getContent(); }
  ...
  placeholder(tab) {
    return `<div style="padding:20px; background-color:#1f2937; border-radius:8px; border:1px solid #374151;">
      ...
      <p style="color:#d1d5db; font-size:14px; line-height:1.5; margin-bottom:16px;">UNCLASSIFIED - Placeholder sustainment practical example content.</p>
    </div>`;
  }
};

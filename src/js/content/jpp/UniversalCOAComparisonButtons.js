/**
 * Universal JPP Step 5: COA Comparison Button System
 * Matches JPP Steps 1-4 visual design and interaction patterns exactly
 */

export class UniversalCOAComparisonButtons {
    constructor() {
        this.navContainerId = 'universal-jpp-coa-comparison-nav';
        this.contentContainerId = 'coa-comparison-content-area';
        this.buttonConfig = [
            {
                id: 'coa-comparison-detailed-analysis-btn',
                moduleType: 'detailed-analysis',
                label: 'Detailed Analysis',
                icon: 'fas fa-sitemap',
                colorClass: 'universal-coa-comparison-btn-green',
                isDefault: true
            },
            {
                id: 'coa-comparison-practical-examples-btn',
                moduleType: 'practical-examples',
                label: 'Practical Examples',
                icon: 'fas fa-book-open',
                colorClass: 'universal-coa-comparison-btn-blue',
                isDefault: false
            },
            {
                id: 'coa-comparison-echelon-analysis-btn',
                moduleType: 'echelon-analysis',
                label: 'Echelon Analysis',
                icon: 'fas fa-layer-group',
                colorClass: 'universal-coa-comparison-btn-purple',
                isDefault: false
            }
        ];
        this.modules = {};
    ...
    injectCSS() {
        ...
        style.textContent = `
            ...
            .universal-coa-comparison-btn-green { background-color: #16a34a !important; color: #fff !important; border-color: #15803d !important; }
            ...
            .universal-coa-comparison-btn-blue { background-color: #2563eb !important; color: #fff !important; border-color: #1d4ed8 !important; }
            .universal-coa-comparison-btn-blue:hover { background-color: #1d4ed8 !important; transform: translateY(-2px) !important; box-shadow: 0 4px 12px rgba(37,99,235,.4) !important; }
            .universal-coa-comparison-btn-purple { background-color: #9333ea !important; color: #fff !important; border-color: #7c3aed !important; }
            .universal-coa-comparison-btn-purple:hover { background-color: #7c3aed !important; transform: translateY(-2px) !important; box-shadow: 0 4px 12px rgba(147,51,234,.4) !important; }
            .universal-coa-comparison-btn-inactive { background-color: #4b5563 !important; color: #9ca3af !important; border-color: #374151 !important; opacity: .7 !important; }
            .universal-coa-comparison-btn-inactive:hover { background-color: #6b7280 !important; transform:none !important; box-shadow:none !important; }
        `;
    ...
    async initialize() {
        try {
            console.log('��� Initializing Universal COA Comparison Button System...');
            this.injectCSS();
            ...
            console.log('✅ Universal COA Comparison Button System initialized successfully');
        } catch (error) {
    ...
    async findContainer() {
        ...
        for (let i = 0; i < 15; i++) {
            // Method 1: direct
            let c = document.getElementById('universal-jpp-coa-comparison-nav');
            if (c) return c;

            // Method 2: querySelector
            c = document.querySelector('#universal-jpp-coa-comparison-nav');
            if (c) return c;

            // Method 3: search inside workspace
            const workspace = document.getElementById('workspace-content');
            if (workspace) {
                c = workspace.querySelector('#universal-jpp-coa-comparison-nav');
                if (c) return c;

                // Method 4: deep search with exact id
                const all = workspace.querySelectorAll('div[id="universal-jpp-coa-comparison-nav"]');
                if (all && all.length > 0) return all[0];
            }

            // Method 5: fallback creation if content area exists
            if (i === 10) {
                const area = workspace?.querySelector('#coa-comparison-content-area');
                if (area && area.parentElement) {
                    const fallback = document.createElement('div');
                    fallback.id = 'universal-jpp-coa-comparison-nav';
        ...
        this.buttonConfig.forEach(btn => {
            ...
            if (button) {
                if (btn.moduleType === activeModuleType) {
                    button.className = `universal-coa-comparison-nav-btn ${activeColorClass}`;
                } else {
                    button.className = `universal-coa-comparison-nav-btn universal-coa-comparison-btn-inactive`;
                }
            }
        });
    }
    ...
    async loadModule(moduleType) {
        ...
        try {
            ...
            if (module && typeof module.getContent === 'function') {
                ...
                console.log(`✅ Loaded COA Comparison module: ${moduleType}`);
            } else {
...
export default UniversalCOAComparisonButtons;

/**
 * JPP Step 2: Mission Analysis - Practical Examples Module
 * Implements hover dropdowns (tactical/operational/strategic) above content; CSP-compliant
 */

export const practicalExamplesModule = {
  id: 'mission-analysis-practical-examples',
  title: 'Practical Examples',
  defaultExpanded: false,

  getContent() {
    return `
      <div id="jpp-mission-analysis-practical-examples" class="dashboard-widgets mb-12">
        ...
        <div style="display:flex; align-items:center; margin-bottom:16px; gap:16px; position:relative; z-index:1500;">
          ...
          ${['tactical','operational','strategic'].map(group => `
            <div class="jpp-ech-dropdown" data-group="${group}" style="position:relative; display:inline-block;">
              <button class="jpp-ech-btn" type="button"
                style="background:none; border:none; color:${group==='tactical'?'#10b981':'#9ca3af'}; font-size:14px; font-weight:600; cursor:pointer; padding:7px 14px; display:flex; align-items:center; transition:color .3s ease;">
                ${group.charAt(0).toUpperCase()+group.slice(1)} Level
                <i class="fas fa-chevron-down" style="margin-left:6px; font-size:12px;"></i>
              </button>
              <div class="jpp-ech-menu"
        ...
        <div class="jpp-mission-examples-area" style="background-color:#1f2937; border-radius:8px; border:1px solid #374151; padding:24px;">
  ...
  getGroupButtons(group) {
    ...
    return map[group].map(b => `
      <button class="jpp-ech-item" data-tab="${b.tab}"
  ...
  initialize() {
    // Inject simple hover CSS
    const styleId = 'jpp-mission-analysis-practical-styles';
    if (!document.getElementById(styleId)) {
      const style = document.createElement('style');
      style.id = styleId;
      style.textContent = `
        .jpp-ech-item:hover { background-color:#374151 !important; }
        .jpp-ech-btn:hover { color:#10b981 !important; }
      `;
      document.head.appendChild(style);
    }

    // Hover open/close behavior
    document.querySelectorAll('.jpp-ech-dropdown').forEach(drop => {
      const menu = drop.querySelector('.jpp-ech-menu');
      if (!menu) return;
    ...
    // Item click handling
    document.querySelectorAll('.jpp-ech-item').forEach(item => {
  ...
  async loadEchelon(tab) {
    const area = document.querySelector('.jpp-mission-examples-area');
    if (!area) return;
    area.innerHTML = `
    ...
    try {
      ...
      console.error('Failed to load practical examples module for', tab, e);
      setTimeout(() => {
  ...
  async importEchelon(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./mission-analysis-practical-platoon.js'); return m.platoonPracticalModule.getContent(); }
      case 'company': { const m = await import('./mission-analysis-practical-company.js'); return m.companyPracticalModule.getContent(); }
      case 'squadron': { const m = await import('./mission-analysis-practical-squadron.js'); return m.squadronPracticalModule.getContent(); }
      case 'battalion': { const m = await import('./mission-analysis-practical-battalion.js'); return m.battalionPracticalModule.getContent(); }
      case 'regiment': { const m = await import('./mission-analysis-practical-regiment.js'); return m.regimentPracticalModule.getContent(); }
      case 'group': { const m = await import('./mission-analysis-practical-group.js'); return m.groupPracticalModule.getContent(); }
      case 'brigade': { const m = await import('./mission-analysis-practical-brigade.js'); return m.brigadePracticalModule.getContent(); }
      case 'division': { const m = await import('./mission-analysis-practical-division.js'); return m.divisionPracticalModule.getContent(); }
      case 'corps': { const m = await import('./mission-analysis-practical-corps.js'); return m.corpsPracticalModule.getContent(); }
      case 'army': { const m = await import('./mission-analysis-practical-army.js'); return m.armyPracticalModule.getContent(); }
      case 'theater': { const m = await import('./mission-analysis-practical-theater.js'); return m.theaterPracticalModule.getContent(); }
      default:

/**
 * JPP Step 5: COA Comparison - Echelon Analysis Module
...
export const echelonAnalysisModule = {
  ...
  getContent() {
    return `
      <div id="jpp-coa-comparison-echelon-analysis" class="dashboard-widgets mb-12">
        <div style="display:flex; align-items:center; margin-bottom:16px; gap:16px; position:relative; z-index:1500;">
          ...
          ${['tactical','operational','strategic'].map(group => `
            <div class="jpp-coa-comparison-ech2-dropdown" data-group="${group}" style="position:relative; display:inline-block;">
              <button id="coa-comparison-${group}-dropdown-btn" class="jpp-coa-comparison-ech2-btn" type="button"
              ...
              <div class="jpp-coa-comparison-ech2-menu"
        ...
        <div class="jpp-coa-comparison-echelon-area" style="background-color:#1f2937; border-radius:8px; border:1px solid #374151; padding:24px;">
  ...
  getGroupButtons(group) {
    ...
    return map[group].map(b => `
      <button class="jpp-coa-comparison-ech2-item" data-tab="${b.tab}"
  ...
  initialize() {
    const styleId = 'jpp-coa-comparison-echelon-styles';
    if (!document.getElementById(styleId)) {
      const style = document.createElement('style');
      style.id = styleId;
      style.textContent = `
        .jpp-coa-comparison-ech2-item:hover { background-color:#374151 !important; }
        .jpp-coa-comparison-ech2-btn:hover { color:#9333ea !important; }
      `;
      document.head.appendChild(style);
    }

    document.querySelectorAll('.jpp-coa-comparison-ech2-dropdown').forEach(drop => {
      const menu = drop.querySelector('.jpp-coa-comparison-ech2-menu');
      if (!menu) return;
    ...
    document.querySelectorAll('.jpp-coa-comparison-ech2-item').forEach(item => {
  ...
  updateDropdownState(active) {
    ...
    ['tactical','operational','strategic'].forEach(g => {
      const btn = document.getElementById(`coa-comparison-${g}-dropdown-btn`);
      if (btn) btn.style.color = (g===activeGroup) ? '#9333ea' : '#9ca3af';
    });
  },

  async loadEchelon(tab) {
    const area = document.querySelector('.jpp-coa-comparison-echelon-area');
    if (!area) return;
  ...
  async getEchelonContent(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./coa-comparison-echelon-platoon.js'); return m.platoonEchelonModule.getContent(); }
      case 'company': { const m = await import('./coa-comparison-echelon-company.js'); return m.companyEchelonModule.getContent(); }
      case 'squadron': { const m = await import('./coa-comparison-echelon-squadron.js'); return m.squadronEchelonModule.getContent(); }
      case 'battalion': { const m = await import('./coa-comparison-echelon-battalion.js'); return m.battalionEchelonModule.getContent(); }
      case 'regiment': { const m = await import('./coa-comparison-echelon-regiment.js'); return m.regimentEchelonModule.getContent(); }
      case 'group': { const m = await import('./coa-comparison-echelon-group.js'); return m.groupEchelonModule.getContent(); }
      case 'brigade': { const m = await import('./coa-comparison-echelon-brigade.js'); return m.brigadeEchelonModule.getContent(); }
      case 'division': { const m = await import('./coa-comparison-echelon-division.js'); return m.divisionEchelonModule.getContent(); }
      case 'corps': { const m = await import('./coa-comparison-echelon-corps.js'); return m.corpsEchelonModule.getContent(); }
      case 'army': { const m = await import('./coa-comparison-echelon-army.js'); return m.armyEchelonModule.getContent(); }
      case 'theater': { const m = await import('./coa-comparison-echelon-theater.js'); return m.theaterEchelonModule.getContent(); }
      default:
    ...
    return `
      <div style="padding:20px; background-color:#1f2937; border-radius:8px; border:1px solid #374151;">
        <h4 style="color:#9333ea; font-size:16px; font-weight:600; margin-bottom:12px;">
          <i class="fas fa-layer-group" style="margin-right:8px;"></i>${label} Level COA Comparison
        </h4>
        <p style="color:#d1d5db; font-size:14px; line-height:1.5; margin-bottom:8px;">UNCLASSIFIED - COA comparison framework for ${label.toLowerCase()} level operations.</p>
        <p style="color:#9ca3af; font-size:12px;">Content follows JP 5-0 COA comparison procedures scaled to command level.</p>
      </div>`;
  }
};

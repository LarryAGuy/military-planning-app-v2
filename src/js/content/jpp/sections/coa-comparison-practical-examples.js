/**
 * JPP Step 5: COA Comparison - Practical Examples Module
...
export const practicalExamplesModule = {
  ...
  getContent() {
    return `
      <div id="jpp-coa-comparison-practical-examples" class="dashboard-widgets mb-12">
        ...
        <div style="display:flex; align-items:center; margin-bottom:16px; gap:16px; position:relative; z-index:1500;">
          ...
          ${['tactical','operational','strategic'].map(group => `
            <div class="jpp-coa-comparison-ech-dropdown" data-group="${group}" style="position:relative; display:inline-block;">
              <button class="jpp-coa-comparison-ech-btn" type="button"
              ...
              <div class="jpp-coa-comparison-ech-menu"
        ...
        <div class="jpp-coa-comparison-examples-area" style="background-color:#1f2937; border-radius:8px; border:1px solid #374151; padding:24px;">
  ...
  getGroupButtons(group) {
    ...
    return map[group].map(b => `
      <button class="jpp-coa-comparison-ech-item" data-tab="${b.tab}"
  ...
  initialize() {
    // Inject hover CSS
    const styleId = 'jpp-coa-comparison-practical-styles';
    if (!document.getElementById(styleId)) {
      const style = document.createElement('style');
      style.id = styleId;
      style.textContent = `
        .jpp-coa-comparison-ech-item:hover { background-color:#374151 !important; }
        .jpp-coa-comparison-ech-btn:hover { color:#3b82f6 !important; }
      `;
      document.head.appendChild(style);
    }

    // Hover open/close behavior
    document.querySelectorAll('.jpp-coa-comparison-ech-dropdown').forEach(drop => {
      const menu = drop.querySelector('.jpp-coa-comparison-ech-menu');
      if (!menu) return;
    ...
    // Item click handling
    document.querySelectorAll('.jpp-coa-comparison-ech-item').forEach(item => {
  ...
  async loadEchelon(tab) {
    const area = document.querySelector('.jpp-coa-comparison-examples-area');
    if (!area) return;
    area.innerHTML = `
    ...
    try {
      ...
      console.error('Failed to load COA comparison practical examples module for', tab, e);
      setTimeout(() => {
  ...
  async importEchelon(tab) {
    switch (tab) {
      case 'platoon': { const m = await import('./coa-comparison-practical-platoon.js'); return m.platoonPracticalModule.getContent(); }
      case 'company': { const m = await import('./coa-comparison-practical-company.js'); return m.companyPracticalModule.getContent(); }
      case 'squadron': { const m = await import('./coa-comparison-practical-squadron.js'); return m.squadronPracticalModule.getContent(); }
      case 'battalion': { const m = await import('./coa-comparison-practical-battalion.js'); return m.battalionPracticalModule.getContent(); }
      case 'regiment': { const m = await import('./coa-comparison-practical-regiment.js'); return m.regimentPracticalModule.getContent(); }
      case 'group': { const m = await import('./coa-comparison-practical-group.js'); return m.groupPracticalModule.getContent(); }
      case 'brigade': { const m = await import('./coa-comparison-practical-brigade.js'); return m.brigadePracticalModule.getContent(); }
      case 'division': { const m = await import('./coa-comparison-practical-division.js'); return m.divisionPracticalModule.getContent(); }
      case 'corps': { const m = await import('./coa-comparison-practical-corps.js'); return m.corpsPracticalModule.getContent(); }
      case 'army': { const m = await import('./coa-comparison-practical-army.js'); return m.armyPracticalModule.getContent(); }
      case 'theater': { const m = await import('./coa-comparison-practical-theater.js'); return m.theaterPracticalModule.getContent(); }
      default:

/**
 * Universal JPP Step 4: COA Analysis Button System
 * Matches JPP Steps 1-3 visual design and interaction patterns exactly
 */

export class UniversalCOAAnalysisButtons {
    constructor() {
        this.navContainerId = 'universal-jpp-coa-analysis-nav';
        this.contentContainerId = 'coa-analysis-content-area';
        this.buttonConfig = [
            {
                id: 'coa-analysis-detailed-analysis-btn',
                moduleType: 'detailed-analysis',
                label: 'Detailed Analysis',
                icon: 'fas fa-sitemap',
                colorClass: 'universal-coa-analysis-btn-green',
                isDefault: true
            },
            {
                id: 'coa-analysis-practical-examples-btn',
                moduleType: 'practical-examples',
                label: 'Practical Examples',
                icon: 'fas fa-book-open',
                colorClass: 'universal-coa-analysis-btn-blue',
                isDefault: false
            },
            {
                id: 'coa-analysis-echelon-analysis-btn',
                moduleType: 'echelon-analysis',
                label: 'Echelon Analysis',
                icon: 'fas fa-layer-group',
                colorClass: 'universal-coa-analysis-btn-purple',
                isDefault: false
            }
        ];
        this.modules = {};
    ...
    injectCSS() {
        ...
        style.textContent = `
            ...
            .universal-coa-analysis-btn-blue { background-color: #2563eb !important; color: #fff !important; border-color: #1d4ed8 !important; }
            .universal-coa-analysis-btn-blue:hover { background-color: #1d4ed8 !important; transform: translateY(-2px) !important; box-shadow: 0 4px 12px rgba(37,99,235,.4) !important; }
            .universal-coa-analysis-btn-purple { background-color: #9333ea !important; color: #fff !important; border-color: #7c3aed !important; }
            .universal-coa-analysis-btn-purple:hover { background-color: #7c3aed !important; transform: translateY(-2px) !important; box-shadow: 0 4px 12px rgba(147,51,234,.4) !important; }
    ...
    async findContainer() {
        ...
        for (let i = 0; i < 15; i++) {
            // Method 1: direct
            let c = document.getElementById('universal-jpp-coa-analysis-nav');
            if (c) return c;

            // Method 2: querySelector
            c = document.querySelector('#universal-jpp-coa-analysis-nav');
            if (c) return c;

            // Method 3: search inside workspace
            const workspace = document.getElementById('workspace-content');
            if (workspace) {
                c = workspace.querySelector('#universal-jpp-coa-analysis-nav');
                if (c) return c;

                // Method 4: deep search with exact id
                const all = workspace.querySelectorAll('div[id="universal-jpp-coa-analysis-nav"]');
                if (all && all.length > 0) return all[0];
            }

            // Method 5: fallback creation if content area exists
            if (i === 10) {
                const area = workspace?.querySelector('#coa-analysis-content-area');
                if (area && area.parentElement) {
                    const fallback = document.createElement('div');
                    fallback.id = 'universal-jpp-coa-analysis-nav';
    ...
    async initialize() {
        try {
            ...
            console.log('✅ Universal COA Analysis Button System initialized successfully');
        ...
        this.buttonConfig.forEach(btn => {
            ...
            if (button) {
                if (btn.moduleType === activeModuleType) {
                    button.className = `universal-coa-analysis-nav-btn ${activeColorClass}`;
                } else {
                    button.className = `universal-coa-analysis-nav-btn universal-coa-analysis-btn-inactive`;
                }
            }
        });
    }
    ...
    async loadModule(moduleType) {
        ...
        try {
            ...
            if (module && typeof module.getContent === 'function') {
                ...
                console.log(`✅ Loaded COA Analysis module: ${moduleType}`);
            } else {
...
export default UniversalCOAAnalysisButtons;

/**
 * Universal JPP Step 3: COA Development Button System
 * Matches JPP Steps 1-2 visual design and interaction patterns exactly
 */

export class UniversalCOADevelopmentButtons {
    constructor() {
        this.navContainerId = 'universal-jpp-coa-development-nav';
        this.contentContainerId = 'coa-development-content-area';
        this.buttonConfig = [
            {
                id: 'coa-detailed-analysis-btn',
                moduleType: 'detailed-analysis',
                label: 'Detailed Analysis',
                icon: 'fas fa-sitemap',
                colorClass: 'universal-coa-btn-green',
                isDefault: true
            },
            {
                id: 'coa-practical-examples-btn',
                moduleType: 'practical-examples',
                label: 'Practical Examples',
                icon: 'fas fa-book-open',
                colorClass: 'universal-coa-btn-blue',
                isDefault: false
            },
            {
                id: 'coa-echelon-analysis-btn',
                moduleType: 'echelon-analysis',
                label: 'Echelon Analysis',
                icon: 'fas fa-layer-group',
                colorClass: 'universal-coa-btn-purple',
                isDefault: false
            }
        ];
        this.modules = {};
    ...
    prefetchSecondaryModules() {
        ...
        const prefetch = () => {
            try {
                import('./sections/coa-development-practical-examples.js');
                import('./sections/coa-development-echelon-analysis.js');
            } catch {}
        };
    ...
    async loadCOADevelopmentModules() {
        const [detailed, practical, echelon] = await Promise.all([
            import('./sections/coa-development-detailed-analysis.js'),
            import('./sections/coa-development-practical-examples.js'),
            import('./sections/coa-development-echelon-analysis.js')
        ]);
        this.modules = {
    ...
    injectCSS() {
        if (document.getElementById('universal-coa-development-styles')) return;
        const style = document.createElement('style');
        style.id = 'universal-coa-development-styles';
        style.textContent = `
            ...
            .universal-coa-btn-blue { background-color: #2563eb !important; color: #fff !important; border-color: #1d4ed8 !important; }
            .universal-coa-btn-blue:hover { background-color: #1d4ed8 !important; transform: translateY(-2px) !important; box-shadow: 0 4px 12px rgba(37,99,235,.4) !important; }
    ...
    async findContainer() {
        ...
        for (let i = 0; i < 15; i++) {
            // Method 1: direct
            let c = document.getElementById('universal-jpp-coa-development-nav');
            if (c) return c;

            // Method 2: querySelector
            c = document.querySelector('#universal-jpp-coa-development-nav');
            if (c) return c;

            // Method 3: search inside workspace
            const workspace = document.getElementById('workspace-content');
            if (workspace) {
                c = workspace.querySelector('#universal-jpp-coa-development-nav');
                if (c) return c;

                // Method 4: deep search with exact id
                const all = workspace.querySelectorAll('div[id="universal-jpp-coa-development-nav"]');
                if (all && all.length > 0) return all[0];
            }

            // Method 5: fallback creation if content area exists
            if (i === 10) {
                const area = workspace?.querySelector('#coa-development-content-area');
                if (area && area.parentElement) {
                    const fallback = document.createElement('div');
                    fallback.id = 'universal-jpp-coa-development-nav';
    ...
    async initialize() {
        try {
            ...
            console.log('✅ Universal COA Development Button System initialized successfully');
        ...
        this.buttonConfig.forEach(btn => {
            ...
            if (button) {
                if (btn.moduleType === activeModuleType) {
                    button.className = `universal-coa-nav-btn ${activeColorClass}`;
                } else {
                    button.className = `universal-coa-nav-btn universal-coa-btn-inactive`;
                }
            }
        });
    }
    ...
    async loadModule(moduleType) {
        ...
        try {
            ...
            if (module && typeof module.getContent === 'function') {
                ...
                console.log(`✅ Loaded COA Development module: ${moduleType}`);
            } else {
...
export default UniversalCOADevelopmentButtons;
